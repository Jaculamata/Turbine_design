...
%排汽温度
   ...
   % p(h_num+1)=0;t(h_num+1)=0;h(h_num+1)=0;p1(h_num+1)=0;hs(h_num+1)=0;ts(h_num+1)=0;hwd(h_num+1)=0;twd(h_num+1)=0;
    getP=open('Thermalgraph1.fig');%新建并打开一个图片文件
    %将指定格式的值输入到屏幕或指定文件
    fprintf('请按顺序在图中点击压力线与热力过程线的交点\n')
    %gcf 返回当前figure对象的句柄值  figure用来显示图形和用户界面的窗口
    %gca返回当前axes对象的句柄值 axes在窗口中显示的图形的轴
    %gco返回当前鼠标单击的句柄值
    %指定figure的尺寸相对单位
